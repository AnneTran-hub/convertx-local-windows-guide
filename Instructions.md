...
This setup runs ConvertX locally through:

```text
127.0.0.1:3000
```

This keeps the ConvertX web interface bound to your own computer.

For private file conversion:

- Do not expose ConvertX through Cloudflare Tunnel, ngrok, or router port forwarding unless you understand the security implications.
- Keep Docker Desktop and ConvertX updated.
- Do not reuse important passwords for the local ConvertX account.
- Local conversion improves privacy, but it does not guarantee that an unknown or malicious input file is safe.
